    ...
    toConfig(ps : string, path : string): void {
        ...
        for (let s in this.subs) {
            if (ps === this.subs[s].ps) {
                ...
                v.Inbound(new inbound('proxy', 10080, '0.0.0.0', PROTOCOL.socks, new socks_inbound(AUTH.noauth)));
                v.addInbound(new inbound('proxy_http', 10809, '0.0.0.0', PROTOCOL.http, new socks_inbound(AUTH.noauth)));

                v.Outbound(new outbound('proxy_out', PROTOCOL.vmess, new vmess_outbound(this.subs[s].ip, Number(this.subs[s].port), this.subs[s].id), new streamSettings(this.subs[s].net, this.subs[s].type)));
                v.addOutbound(new outbound('direct', PROTOCOL.freedom, null, null));
                v.addOutbound(new outbound('block', PROTOCOL.blackhole, null, null));

                let routing = v.Routing(DOMAINSTRATEGY.IPIfNonMatch);
                routing.addRules(new ruleObject(null, null, ['api'], 'api',));
                ...
                routing.addRules(new ruleObject(["geosite:category-ads-all"], null, null, 'block'));

                try {
                    fs.writeFile(path + '/config.json', JSON.stringify(v, null, 4), () => {});
                } catch (err) {
                    console.log(err);
                }
                break;
